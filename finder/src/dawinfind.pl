...
if ( defined($myopts{d}) ) {
	...
	print "검색 directory --> @dir\n";
} else { @dir = ('.') ; }

if ( defined($myopts{v}) ) {
	die "!! -v 지정한 파일이 없습니다.($myopts{v})" unless -e $myopts{v};
	$g_std_time = (stat($myopts{v}))[9] ;
	print "기준일시:[ ", Dawin::to_strdate($g_std_time,"-",":")," ]\n";
}
my $delim = $myopts{r} || ';' ;
$dirs = join ('|',@dir) ;
if ( defined($myopts{o}) ) {
...
if ( defined($myopts{f}) ) {
	$sfile = $myopts{f} ;
  die "!! 지정한 파일이 없습니다.($sfile)" unless -e $sfile;
  if ( defined($myopts{k}) ) {
    read_funcName($sfile) ;
...
if  ( defined($myopts{k}) ) {
	$Fn1 = \&inspect_func ;
	print $FHW " DIR \t 파일명 \t Function \t 호출DIR \t \호출프로그램 \t 줄번호 \t 검색내용 \n" ;
} elsif  ( defined($myopts{t}) ) {
	$Fn1 = \&inspect_file_2 ;
	print $FHW " TBL \t COL \t DIR \t 파일명 \t 줄번호 \t 검색내용 \t DML \t 함수명 \n" ;
} else {
	$Fn1 = \&inspect_file_new ;
	print $FHW " 검색항목 \t DIR \t 파일명 \t 줄번호 \t 검색내용 \t DML \t 함수명 \n" ;
}
...
Usage: $0 [-acijkmnst][-o 결과파일][-d 검색DIR] [-v 기준파일 ] [-f 검색항목파일] [-e 제외파일] [-g 포함파일]

OPTION
 -a : 모든파일을 검색대상으로 함
 -c : 주석포함검색
 -d : 검색DIR ( 생략시 현재 디렉토리 검색 )
 -e 제외확장자 : 검색대상제외 파일확장자 ( ex : -e txt,log )
 -f 검색항목파일 : 검색값이 기록된 텍스트 파일
 -g 포함확장자 : 검색대상포함 파일확장자 ( ex : -g txt,log )
 -i : 대소문자 무시
 -j : utf8 => euc-kr 변환
 -k : 검색결과의 함수를 사용하는 파일 검색 -kt, -k
 -m : SELECT * INSERT * 검색
 -n : 퀴리분석안함 ( 단순검색 )
 ...
 -s : 최소검색 ( 동일내용은 1회만 표시 )
 -r 문단구분 : 문단구분자 지정 (기본값 ;)
 -t : 테이블,칼럼 2항목으로 검색(없으면 1항목검색)
 -v 기준파일 : 기준파일 이전 변경된 파일 무시 
